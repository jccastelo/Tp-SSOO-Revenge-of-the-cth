...
void kernel_servers_init(void) {
    ...
    setup_server("DISPATCH",config_kernel->IP_SERVER, string_itoa(config_kernel->PUERTO_ESCUCHA_DISPATCH),  set_dispatcher_id);
    setup_server("I0",config_kernel->IP_SERVER, string_itoa(config_kernel->PUERTO_ESCUCHA_IO), set_io_id);
    setup_server("INTERRUPT",config_kernel->IP_SERVER, string_itoa(config_kernel->PUERTO_ESCUCHA_INTERRUPT), set_interrupt_id);
}
